...
static double pgl_monte_carlo_area(pgl_cluster *cluster, int samples) {
  ...
  for (i=0; i<samples; i++) {
    if (pgl_point_in_cluster(points+i, cluster, true)) matches++;
  }
  pfree(points);
  return area * ((double)matches / samples);
}
...
static double pgl_fair_distance(
  ...
  pgl_point *points;     /* sample points for Monte Carlo method */
  double area;           /* area covered by sample points */
  int i;
  int matches = 0;       /* number of points within enlarged area (multiplied
                            by two to be able to store half matches) */
  double result;         /* result determined by Monte Carlo method */
  ...
  if (distance > 0) {
    ...
    for (i=0; i<samples; i++) {
      /* count sample poitns within cluster as half match */
      if (pgl_point_in_cluster(points+i, cluster, true)) matches += 1;
      /* count sample points outside of cluster but within cluster grown by
         distance as full match */
      else if (
        pgl_point_cluster_distance(points+i, cluster) < distance
      ) matches += 2;
    }
  } else {
    /* if point is within cluster,
       just count sample points within cluster as half match */
    for (i=0; i<samples; i++) {
      if (pgl_point_in_cluster(points+i, cluster, true)) matches += 1;
    }
  }
  /* release memory for sample points needed by Monte Carlo method */
  pfree(points);
  /* convert area determined by Monte Carlo method into distance
     (i.e. radius of circle with the same area) */
  result = sqrt(area * ((double)matches / 2.0 / samples) / M_PI);
